...
export async function GET(req:NextRequest){
 try{
  ...
  if(coreOnly){
   const [sales,directors]=await Promise.all([salesP,directorsP]);const err=sales.error||directors.error;if(err)throw err;
   return NextResponse.json({sales:(sales.data||[]).sort((a:any,b:any)=>alpha(a.name,b.name)),directors:(directors.data||[]).map((x:any)=>({...x,signature_path:'/signature-mr-herry.png'})).sort((a:any,b:any)=>alpha(a.name,b.name)),uoms:STANDARD_UOMS});
  }
  const [sales,members,directors]=await Promise.all([salesP,sb.from('member_directory').select('id,member_name,op_unit_name,client_name,address,active').eq('active',true).order('op_unit_name').order('member_name').limit(20000),directorsP]);
