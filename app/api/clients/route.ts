...
async function batchAdd(rows:BatchRow[]){
 const sb=getSupabaseServerClient();
 const cleaned=(rows||[]).slice(0,500).map(r=>({client:clean(r.client),address:clean(r.address),attention:clean(r.attention),active:r.active!==false})).filter(r=>r.client&&r.attention);
 if(!cleaned.length)throw new Error('Paste at least one Client + Attention row.');
 const {data:existing,error}=await sb.from('member_directory').select('id,op_unit_name,client_name,member_name,address,active').limit(10000);if(error)throw error;
 const existingRows=(existing||[]) as any[];
 const byClient=new Map<string,any[]>();
 for(const row of existingRows){const key=norm(row.op_unit_name);const arr=byClient.get(key)||[];arr.push(row);byClient.set(key,arr)}
 const grouped=new Map<string,{client:string;address:string;rows:{client:string;address:string;attention:string;active:boolean}[]}>();
 ...
 for(const group of grouped.values()){
  ...
  // Update existing client rows in one later upsert if the shared address changed.
  if(old.length&&address){for(const x of old){if(clean(x.address)!==address)updates.push({...x,op_unit_name:canonical,client_name:canonical,address,updated_at:stamp})}}
  for(const r of group.rows){
   const k=norm(r.attention);if(!k||seen.has(k)){skipped++;continue}
   inserts.push({member_name:r.attention,op_unit_name:canonical,client_name:canonical,address,active:r.active!==false,updated_at:stamp});seen.add(k)
  }
 }
 // Supabase accepts arrays: 500 pasted rows now need only a few DB round-trips instead of hundreds.
 if(updates.length){const {error:ue}=await sb.from('member_directory').upsert(updates,{onConflict:'id'});if(ue)throw ue}
 if(inserts.length){
  // Keep payloads reasonably sized while still reducing hundreds of sequential requests.
  for(let i=0;i<inserts.length;i+=500){const {error:ie}=await sb.from('member_directory').insert(inserts.slice(i,i+500));if(ie)throw ie}
 }
