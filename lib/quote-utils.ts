...
export function cleanItems(items:QuoteItem[]){return (items||[]).map(x=>({code:String(x.code||'').trim(),productName:String(x.productName||'').trim(),spec:String(x.spec||'').trim(),brand:String(x.brand||'').trim(),user:String(x.user||'').trim(),leadTime:String(x.leadTime||'').trim(),qty:Number(x.qty||0),uom:String(x.uom||'EA').trim().toUpperCase(),unitPrice:Number(x.unitPrice||0),remarks:String(x.remarks||'').trim()})).filter(x=>x.code||x.productName||x.spec||x.brand||x.user||x.qty||x.unitPrice||x.remarks)}
