-- Complete setup for a new Serveone Quotation V3.1 Supabase project.
...
create unique index if not exists member_directory_mem_uq
  on public.member_directory(mem_id) where mem_id is not null;
...
-- Signature is stored in the application repository (public/signature-mr-herry.png).
-- No Supabase Storage bucket is required for this edition.
