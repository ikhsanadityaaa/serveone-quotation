...
export default function HistoryApp(){
 ...
 async function download(){setMsg('');setLoadingLabel('Preparing Excel...');setLoading(true);try{const r=await fetch('/api/history/export',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({ids:filtered.map(x=>x.id)})});if(!r.ok)return setMsg(await r.text());const b=await r.blob(),u=URL.createObjectURL(b),a=document.createElement('a');a.href=u;a.download='quotation-list.xlsx';a.click();URL.revokeObjectURL(u)}finally{setLoading(false)}}
 async function deleteSelected(){if(!selectedQuotes.length)return;const names=selectedQuotes.slice(0,5).map(q=>q.quotation_no).join('\n');const more=selectedQuotes.length>5?`\n...and ${selectedQuotes.length-5} more`:'';if(!confirm(`Delete ${selectedQuotes.length} selected quotation(s)?\n\n${names}${more}\n\nDeleted quotation numbers will NOT be reused.`))return;setMsg('');setLoadingLabel('Deleting selected quotations...');setLoading(true);try{const r=await fetch('/api/history/batch',{method:'DELETE',headers:{'content-type':'application/json'},body:JSON.stringify({ids:selectedQuotes.map(q=>q.id)})});const j=await r.json().catch(()=>({}));if(!r.ok)return setMsg(j.error||'Failed to delete selected quotations');const deleted=new Set(selectedQuotes.map(q=>q.id));setRows(current=>current.filter(q=>!deleted.has(q.id)));setSelectedIds(new Set());setMsg(`${selectedQuotes.length} quotation(s) deleted. Running numbers remain reserved.`)}finally{setLoading(false)}}
 ...
 <td className="rowno">{itemIndex+1}</td><td>{item?.productName||''}</td><td className="spec-cell">{item?.spec||''}</td><td>{item?.brand||''}</td><td>{item?.user||''}</td><td className="center-cell">{item?.leadTime||''}</td><td className="num">{item?.qty||''}</td><td className="center-cell">{item?.uom||''}</td><td className="num">{item?`IDR ${idr.format(item.unitPrice||0)}`:''}</td><td className="num">{item?`IDR ${idr.format((item.qty||0)*(item.unitPrice||0))}`:''}</td><td className="history-remark readonly-remark">{item?.remarks||''}</td>
 </tr>}) :<tr><td colSpan={18} className="empty-table">No quotation matches the current filters.</td></tr>}</tbody></table></div>
