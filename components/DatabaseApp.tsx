...
export default function DatabaseApp(){
 ...
 return <>
  ...
  <section className="panel db-section company-settings-section"><div className="panel-title"><div><h2>Serveone Information</h2><p>The saved address is used for new quotations and their PDF/Excel output.</p></div></div><div className="company-settings-grid"><label>Company<input value={COMPANY_NAME} readOnly/></label><label>Serveone Address<textarea rows={3} value={companyAddress} onChange={e=>setCompanyAddress(e.target.value)} placeholder="Serveone issuer address"/></label></div><div className="company-settings-actions"><button className="btn primary" disabled={busy} onClick={saveCompanyAddress}>Save Address</button></div></section>
